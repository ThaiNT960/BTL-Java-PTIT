...
public class OrderDAO {
    ...
    public String create(Order order, List<OrderItem> items) {
        ...
        try (Connection conn = getConnection();
            ...
            // Chèn order_items và Cập nhật kho
            for (OrderItem it : items) {
                // 1. Chuẩn bị chèn item
            ...
            // (Lưu ý: Nếu có lỗi, transaction sẽ tự động rollback khi connection bị đóng)
            return null;
        }
    }
...
//     Danh sách đơn theo filter (status/phone/q) + paging; mặc định sort created_at DESC
    ...
    // [CODE THÊM VÀO]
    // Tính tổng doanh thu (chỉ các đơn đã giao)
    public double calculateTotalRevenue() throws SQLException {
        // Chỉ tính doanh thu các đơn 'delivered'
    ...
    // Trả về Map chứa "name" (tên SP) và "count" (số lượng đã bán)
    public Map<String, Object> findBestSellingProduct() throws SQLException {
        // Đây là code SQL Java 8, thay thế cho """
        String sql = "SELECT "
        ...
        try (Connection conn = getConnection();
            ...
            if (rs.next()) {
                ...
                result.put("name", rs.getString("product_name_snapshot"));
                result.put("count", rs.getInt("total_sold"));
                return result;
            }
        }
