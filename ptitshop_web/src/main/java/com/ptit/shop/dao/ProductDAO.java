...
public class ProductDAO {
    ...
    // [CODE THÊM VÀO]
    // Đếm tổng số danh mục (khác nhau)
    public int countDistinctCategories() throws SQLException {
    ...
    // Top sản phẩm bán chạy nhất theo số lượng trong order_items
    public List<Product> listBestSellers(int limit) throws SQLException {
        String sql = """
        SELECT p.id, p.name, p.category, p.price, p.description, p.sizes,
               p.image, p.stock,
               SUM(oi.quantity) AS total_sold
        FROM order_items oi
        JOIN products p ON oi.product_id = p.id
        GROUP BY p.id, p.name, p.category, p.price, p.description, p.sizes, p.image, p.stock
