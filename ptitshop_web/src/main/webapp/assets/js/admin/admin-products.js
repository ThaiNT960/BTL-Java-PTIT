...
(function () {
    ...
    async function loadProductsTable() {
        ...
        // ĐÃ SỬA: colspan = 7 vì có thêm cột tồn kho
        tableBody.innerHTML = `
      ...
      <tr class="border-b border-gray-200 hover:bg-gray-50">
        ...
        <!-- ✅ CỘT TỒN KHO MỚI -->
        <td class="px-6 py-4 text-sm">${(product.stock != null ? product.stock : 0)}</td>
        <!-- Mô tả -->
        <td class="px-6 py-4 text-sm text-gray-600">
            ${(product.description || '').substring(0, 40)}...
        </td>
        <td class="px-6 py-4 text-sm">
          <button class="text-blue-600 hover:text-blue-800 mr-4" onclick="editProduct(${product.id})">
    ...
    window.editProduct = async function (id) {
        ...
        // ✅ GÁN GIÁ TRỊ TỒN KHO VÀO INPUT
        if ($('productStock'))       $('productStock').value       = (p.stock != null ? p.stock : 0);
    ...
    document.getElementById('productForm')?.addEventListener('submit', async function (e) {
        ...
        const payload = {
            ...
            sizes:       sizesCsv,
            stock:       stockValue     // ✅ GỬI STOCK LÊN SERVER
        };
